  ...
  \begin{frame}[t]{Modelagem como sistema linear}
    \begin{itemize}
      ...
      \item
        ...
        Sistema Ax=b, com 'número de nós' equações, em que a matriz A
...
\end{tikzpicture}
  ...
  \begin{frame}{Linearização do custo convexo}
    \begin{itemize}[<+->]
      \item
        Após a linearização pode-se usar o GLPK
      \item
        Resolve-se o problema de custo mínimo
      \item
        Há duas opções: out-of-kilter (okalg) e relaxation4 (relax4)
  ...
  \begin{frame}{okalg}
    \begin{itemize}[<+->]
      ...
      \item
        ideia geral: similar ao caminho mínimo
        kilter number: mínimo aumento ou decremento para satisfazer
        restrições de fluxo e de viabilidade do dual
        
        iterativo até que todos os arcos estejam em 'ordem' (in kilter)
      \item
  ...
  \begin{frame}{relax4}
    \begin{itemize}[<+->]
      \item
        um dos mais eficientes para otimização em redes. 1985 Bertsekas. 1988 Bertsekas and Tseng.
       \item
        ideia geral: pseudoFlow, iteração baseada em aumento do fluxo e ajuste de 'preço' do dual.
      \item
...
\section{Resultados}
  ...
  \subsection{Rede Exemplo}
    ...
    \centering
    Assumindo resistências centenárias.
    \begin{circuitikz}[nodestyle/.style={circle,draw, fill=gray!50}, corrente/.style={very thick, shorten <= 4pt, shorten >= 4pt}, scale=0.85]
    ...
    \end{circuitikz}
	  ...
	  \begin{frame}{Minimização Quadrática - Octave}
			\centering
			\scalebox{0.6} {
			\begin {tabular}{|c|c|c|c|c|c|c|c|c|c|c|c|}
			\hline
    ...
    \begin{frame}{Fluxo em Redes - Custo Mínimo - Out-of-Kilter - GLPK}
			\centering
  ...
  \subsection{Outras redes}
    \begin{frame}{Usando o net genenerator Klingman }
			...
			\hline
				amostra & octave & nr & okalg & relax4 & nós & arcos \\ \hline
			...
			\\
			\small{* Octave - arquivo de entrada excedeu um tamanho razoável.}
			\\
			\small{** okalg e relax4 - arcos com segmentação 100}
			\\
			\small{*** okalg e relax4 - arcos com segmentação 1}
			\\
			\small{Tempo em (ms)}
		\end{frame}
  ...
  \begin{frame}
    ...
    \begin{itemize}[<+->]
      ...
      \item
        ...
        o resultado inteiro é uma desvantagem aparente do método de custo mínimo.        
        Recompensada pelo baixo custo computacional.
      \item
        Octave e nr não escalam. okalg é muito bom, no entanto o relax4
        é o algoritmo atualmente a ser batido.
